...
export class ConnectionManager {
    constructor(tokenManager, stateManager, toastService, configService, options = {}) {
    ...
    async connect() {
        ...
        try {
            ...
            return handleError(errorMessage, 'ConnectionProcess', 'high').success;
        }
    }
    ...
    async attemptRecovery(reason = 'manual') {
        if (this.isDisposed)
            return false;
        const authState = this.stateManager.getAuthState();
        if (!authState.isAuthenticated) {
    ...
    handleDeviceIdInvalidation(source, reason) {
        if (this.isDisposed)
            return;
