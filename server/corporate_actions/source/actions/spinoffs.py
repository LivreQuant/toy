...
class EnhancedSpinoffProcessor:
    ...
    def export_unified_spinoffs(self, results, data_dir=None, filename=None):
        """Export unified spinoffs to CSV format"""
        if data_dir is None:
            data_dir = config.data_dir
            
        if filename is None:
            filename = config.get_unified_filename(config.unified_spinoffs_file)
