...
from .utils import get_token_from_request # Import from the new utils file
...
async def handle_session_ready(request):
    ...
    with optional_trace_span(_tracer, "handle_session_ready") as span:
        ...
        session_id = request.match_info.get('session_id')
        if not session_id:
             return web.json_response({'success': False,'status': 'bad_request', 'message': 'Missing session_id in path'}, status=400)
        span.set_attribute("session_id", session_id)
