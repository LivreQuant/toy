...
class StreamManager:
    ...
    async def stop_stream(self, session_id: str):
        ...
        if task and not task.done():
            ...
            try:
                ...
                logger.error(f"Error awaiting cancelled stream task for session {session_id}: {e}", exc_info=False) # Keep log concise
    ...
    def _handle_task_completion(self, session_id: str, task: asyncio.Task):
        ...
        if registered_task is task:
            logger.debug(f"Stream task completed or cancelled for session {session_id}. Removing from active streams. Task: {task.get_name()}")
        ...
        try:
            ...
            if exception:
                logger.error(f"Managed stream task for session {session_id} finished with error: {exception}", exc_info=exception)
        except asyncio.CancelledError:
