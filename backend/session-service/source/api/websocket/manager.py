...
from source.api.websocket.stream_manager import StreamManager

from source.api.websocket.emitters import connection_emitter, error_emitter, exchange_emitter
...
class WebSocketManager:
    ...
    async def broadcast_to_session(self, session_id: str, message: Any):
        ...
        if isinstance(message, dict):
            data = json.dumps(message); message_type = message.get('type', 'unknown')
        else:
            data = str(message); message_type = 'raw_string'
    ...
    async def _cleanup_stale_connections(self, registry: WebSocketRegistry):
        ...
        while True:
            try:
                ...
                logger.info("Cleanup task cancelled."); break
            except Exception as e:
                logger.error(f"Error in cleanup task: {e}", exc_info=True); await asyncio.sleep(60)
