...
class MetadataManager(BaseTableManager):
    ...
    async def load_exchange_metadata(self, group_id: str = None) -> Dict:
        ...
        try:
            async with self.pool.acquire() as conn:
                if group_id:
                    query = """
                        SELECT exch_id, group_id, exchange_type, timezone, exchanges, 
                ...
                if row:
                    ...
                    self.logger.info(f"✅ Loaded exchange metadata for group_id: {group_id}")
                    return metadata
                else:
                    self.logger.warning(f"⚠️ No exchange metadata found for group_id: {group_id}")
                    return {}
            ...
            async with self.pool.acquire() as conn:
                query = """
                    INSERT INTO exch_us_equity.metadata (
                        exch_id, group_id, exchange_type, timezone, exchanges, 
                        last_snap, pre_market_open, market_open, market_close, 
                        post_market_close
                ...
                self.logger.info(f"✅ Inserted/updated exchange metadata for group_id: {metadata['group_id']}")
                return True
