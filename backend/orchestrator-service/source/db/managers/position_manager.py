...
class PositionManager(BaseManager):
    ...
    async def initialize_tables(self):
        ...
        await self.create_schema_if_not_exists('positions')
        ...
        await self.execute("""
            CREATE TABLE IF NOT EXISTS positions.eod_prices (
                ...
                created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
                updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
                UNIQUE(symbol, price_date, pricing_source)
            )
        """)
        
        # Create indexes
        await self.execute("CREATE INDEX IF NOT EXISTS idx_current_positions_account_date ON positions.current_positions(account_id, position_date)")
        await self.execute("CREATE INDEX IF NOT EXISTS idx_position_history_date ON positions.position_history(position_date)")
        await self.execute("CREATE INDEX IF NOT EXISTS idx_eod_prices_symbol_date ON positions.eod_prices(symbol, price_date)")
